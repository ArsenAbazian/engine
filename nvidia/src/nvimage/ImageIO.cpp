...
bool nv::ImageIO::saveBMP(char *fileName, const Image * img) { 
	...
	DWORD					bytes = img->width() * 4;
	
	if( bytes & 0x03 )
		bytes = (bytes & 0xfffffffc) + 4;
	...
	bih.biSizeImage			= bytes * img->height();
	bih.biXPelsPerMeter		= 0;
	bih.biYPelsPerMeter		= 0;
	bih.biClrUsed			= 0;
	bih.biClrImportant		= 0;

	fopen_s( &fp, fileName, TEXT("wb") );
	if(fp == NULL)
		return false;
	...
	nv::Color32 *buf = (Color32*)img->pixels();
	buf += (img->height() - 1) * img->width();
	for(i = img->height() - 1; i >= 0; i--){
		fwrite((BYTE*)buf, 1, bytes, fp);
		buf -= img->width();
	}
